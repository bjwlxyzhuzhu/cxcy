...
export default function AccountPanel({
  ...
  const [adv, setAdv] = useState<Record<Purpose, boolean>>({
    chat: false,
    reason: false,
    text: false,
    image: false,
  ...
  async function saveKey(p: Purpose) {
    ...
    if (error) setMsg("保存失败：" + error.message);
      ...
      setKey(p, { saved: true });
      setMsg(
        `已保存「${PURPOSES.find((x) => x.key === p)?.label}」的 API Key（将用你自己的额度，不扣积分）`,
      );
    }
  }
        ...
        >
          <div
            style={{
              ...
              placeItems: "center",
              fontSize: 26,
              background: "linear-gradient(135deg,#34507a,#22d3ee)",
            }}
          >
            👤
          </div>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div
              style={{ fontWeight: 800, fontSize: 20, color: "var(--cyan)" }}
            >
              {profile.credits ?? "—"}
            </div>
          </div>
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <div>
                ...
                <button
                  onClick={() =>
                    (
                      window as unknown as { __openAvatarPicker?: () => void }
                    ).__openAvatarPicker?.()
                  }
                  style={btn("rgba(255,255,255,.08)")}
                >
                  🐼 更换星际头像
                </button>
              </div>
              ...
              >
                <p>
                  <a href="/me/records" style={{ color: "var(--cyan)" }}>
                    查看我的测试记录与导出
                  </a>
                </p>
                <button
          ...
          {tab === "keys" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div
                style={{ fontSize: 12, color: "var(--mut)", lineHeight: 1.7 }}
              >
                为各智能体分别绑定
                <b style={{ color: "var(--ink)" }}>国内大模型</b>（配图可另选
                GPT Image 2）。
                <b style={{ color: "var(--cyan)" }}>协议已写好</b>，你只需
                <b style={{ color: "var(--ink)" }}>选模型 + 粘贴 API Key</b>。
                <br />
                <b style={{ color: "var(--ink)" }}>留空</b> =
                用平台默认并扣积分；<b style={{ color: "var(--cyan)" }}>填了</b>{" "}
                = 用你自己额度、<b style={{ color: "var(--cyan)" }}>不扣积分</b>
                。
              </div>
              {PURPOSES.map(({ key, icon, label, agent, desc }) => {
                ...
                const mkOpt = (p: (typeof PROVIDERS)[number]) => ({
                  value: p.id,
                  label: p.short,
                });
                const modelGroups = [
                ...
                return (
                  ...
                  >
                    ...
                    <ModelSelect
                      value={k.provider}
                      onChange={(id) => pickProvider(key, id)}
                      groups={modelGroups}
                      placeholder="选择模型…（自动写好协议）"
                    />
                    ...
                    >
                      ② 粘贴该模型的 API Key（只需填这一项）
                    </div>
                    <input
                      type="password"
                      value={k.api_key}
                      onChange={(e) => setKey(key, { api_key: e.target.value })}
                    ...
                    {adv[key] && (
                      <div style={{ marginTop: 8 }}>
                        <input
                          value={k.base_url}
                          onChange={(e) =>
                            setKey(key, { base_url: e.target.value })
                          }
                        ...
                        <input
                          value={k.model}
                          onChange={(e) =>
                            setKey(key, { model: e.target.value })
                          }
                          placeholder="模型名 model"
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                      <button
                        onClick={() => saveKey(key)}
                        disabled={busy || !k.api_key || !k.base_url}
                        style={btn(
                          "linear-gradient(120deg,var(--cyan),var(--violet))",
                        )}
                      >
          ...
          {tab === "bill" && (
            <div>
              ...
              >
                ...
                <span>
                  累计消耗 <b style={{ color: "var(--cyan)" }}>{totalCost}</b>{" "}
                  分
                </span>
              </div>
              {usage.length === 0 ? (
                ...
                >
                  {usage.map((u, i) => (
                    ...
                    >
                      ...
                      <span
                        style={{ flex: 1, color: "var(--mut)", fontSize: 11.5 }}
                      >
