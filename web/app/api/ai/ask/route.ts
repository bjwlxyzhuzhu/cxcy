...
export async function POST(req: Request) {
  ...
  const maxTokens =
    Number.isFinite(mtRaw) && mtRaw > 0
      ? Math.min(8192, Math.max(512, Math.round(mtRaw)))
      : undefined;
  ...
  if (!lastUserTxt.trim() && !docText && !images.length)
    return NextResponse.json({ error: "请输入内容" }, { status: 400 });
  let record: Awaited<ReturnType<typeof startRecord>>;
  try {
    const moduleKey = moduleFromRequest(req);
    record = await startRecord(
      user.id,
      moduleKey,
      body.evidence?.title || MODULES[moduleKey],
      body.sessionId,
      body.requestId,
      lastUserTxt,
      {
        ...
        imageNames: images.map((a) => a.name),
        entryModule: moduleKey,
      },
    );
    if (record.cached)
      return NextResponse.json({
    ...
    return NextResponse.json(
      { error: "记录未能保存，尚未调用AI。请重试，保留当前输入。" },
      { status: 503 },
    );
  }
  ...
  if (lastUser) {
    try {
      const emb = await getApimart().embeddings.create({
        model: EMBED_MODEL,
        input: lastUser,
      });
      ...
      if (good.length) {
        context = good
          .map(
            (h, i) => `【资料${i + 1}·${h.source}/${h.title || ""}】${h.chunk}`,
          )
          .join("\n");
        ...
        for (const h of good) {
          ...
          if (!seen.has(k)) {
            seen.add(k);
            sources.push({ title: h.title || "", source: h.source || "" });
          }
        }
      }
    } catch {
  ...
  if (wantWeb && lastUser) {
    ...
    if (hit) {
      ...
      for (const it of hit.items)
        if (it.title) sources.push({ title: it.title, source: "联网检索" });
    }
  }
  ...
  if (plugins.includes("webfetch") && lastUserTxt) {
    ...
    for (const f of fetched)
      if (f.ok) sources.push({ title: f.url, source: "网页抓取" });
  }
  ...
  if (docText || images.length) {
    ...
    if (li >= 0) {
      const base =
        typeof outMsgs[li].content === "string"
          ? (outMsgs[li].content as string)
          : "";
      const withDoc = docText ? `${base}\n\n${docText}` : base;
      outMsgs[li].content = images.length
        ? [
            { type: "text", text: withDoc },
            ...images.map((im) => ({
              type: "image_url",
  ...
  const complete = async (mdl: string) => {
    const stream = await client.chat.completions.create({
      model: mdl,
      messages: [...sysMsgs, ...outMsgs] as never,
      stream: true,
  ...
  try {
    ...
    if (overrideModel && overrideModel !== model) {
      try {
        text = await complete(model);
        usedModel = model;
      } catch (e2) {
        return NextResponse.json(
          {
            error:
              "AI 调用失败：" + (e2 instanceof Error ? e2.message : "未知错误"),
          },
          { status: 502 },
        );
      }
  ...
  if (evKind && ASK_GATES[evKind] && ASK_GATES[evKind](text)) {
    const dims =
      evKind === "defense_radar" ? parseRadar(text, ev?.rubric) : null;
    if (dims && ev?.meta && typeof ev.meta.key === "string")
      dims.key = ev.meta.key;
  ...
  try {
    await finishRecord(record.sessionId, record.requestId, text, {
      model: usedModel,
      sources,
      source: "ai",
    });
  ...
  const r = await deductCredits(user.id, "chat", COST.chat, {
    model: usedModel,
    rag: sources.length,
    web: webUsed,
  });
  return NextResponse.json({
    text,
    remaining: r.ok ? r.remaining : credits,
    ownKey: false,
    sources,
    ...meta,
  });
