...
const COCKPIT_STEPS: TourStep[] = [
  ...
  },
  {
    ...
    badge: "②",
    title: "按需增删搭子",
    body: "右侧轮盘可加入更多专家搭子（熊猫总负责 + 材料撰写固定保留）；甲板上点搭子可移出。工作流会按所选自动编排、材料撰写恒在最后打包。",
  },
  {
    sel: "#ck-goal",
    badge: "③",
  ...
  },
  {
    ...
    badge: "④",
    title: "模型 · 篇幅 · 技能 · 语音",
    body: "选大模型与成稿篇幅；点「🧩 技能」加载技能、「🎤」语音输入。下方还能进「🛒 技能商店」装更多技能、用「☁ 我的项目」云端存取。",
  },
  {
    sel: "#ck-start",
    badge: "⑤",
    title: "一键开始协作",
    body: "点这里，智能体集群开始协同作战，自动产出一整套商业计划书；完成后可「查看 / 导出成稿」为 Word / PDF / 路演 PPT。",
  },
];
    ...
    return (
      <div
        style={{
          ...
          placeItems: "center",
          fontSize: size * 0.5,
          background: `radial-gradient(circle at 38% 32%, ${color}55, ${color}1a 70%, rgba(7,9,26,.5))`,
        }}
      >
        {emoji}
      </div>
...
export default function CockpitPage() {
  ...
  useEffect(() => {
    ...
    if (id) {
      recordId.current = id;
      setSavedRecord(id);
      fetch("/api/records?sessionId=" + id)
        .then((r) => r.json())
        .then((d) => {
          if (d.error) {
            setErr(d.error);
          ...
          const last = d.records
            ?.filter((r: { role: string }) => r.role === "assistant")
            .at(-1);
          if (last) setDraft(last.content);
        })
        .catch(() => setErr("云端记录读取失败，请从历史记录重试"));
    }
  }, []);
  ...
  const [ownChat, setOwnChat] = useState<{
    short: string;
    model: string;
  } | null>(null); // 学生在账户中心绑定的「对话」模型（有则自带 Key·不扣积分）
  ...
  useEffect(() => {
    ...
    try {
      const tr = localStorage.getItem(TEAM_LS);
      if (tr) {
        const p = JSON.parse(tr);
        const saved = (p.team || []).filter((k: string) => CREW_BY_KEY[k]);
        const tm = Array.from(new Set<string>(["boss", ...saved, "advisor"]));
        setTeam(tm);
        setLeader(tm.includes(p.leader) ? p.leader : "boss");
      }
    } catch {
      /* ignore */
    }
    try {
      if (typeof window !== "undefined" && !sessionStorage.getItem(INTRO_SS))
        setIntro(true);
    } catch {
      /* ignore */
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem(TEAM_LS, JSON.stringify({ team, leader }));
    } catch {
      /* ignore */
    }
  }, [team, leader]);
  useEffect(() => {
    try {
      ...
      localStorage.setItem(LS, JSON.stringify({ ...p, name, draft }));
    } catch {
      /* ignore */
    }
  }, [name, draft]);
    ...
    (async () => {
      try {
        ...
        const { data } = await api
          .from<{ provider?: string; model?: string }>("user_api_keys")
        ...
        if (data) {
          const short =
            PROVIDERS.find((p) => p.id === data.provider)?.short ||
            data.model ||
            "我的模型";
  ...
  const T = dark
    ...
    : {
        ...
        line: "rgba(80,120,200,.28)",
        inBg: "rgba(255,255,255,.9)",
        inLine: "rgba(80,120,200,.3)",
        pageBg: "#e9eefb",
      };
  ...
  function toggleMember(k: string) {
    ...
    setTeam((t) => {
      if (t.includes(k)) {
        const n = t.filter((x) => x !== k);
        if (!n.length) return t;
        if (k === leader) setLeader(n.includes("boss") ? "boss" : n[0]);
        return n;
      }
      return [...t, k];
    });
  ...
  const modelOpts = ownChat
    ? [{ v: "", label: `🔑 我的 ${ownChat.short} · 不扣积分` }]
    : PLATFORM_MODELS;
  ...
  async function ask(
    ...
    useWeb = false,
    maxTokens?: number,
    evidence?: { kind: string; title?: string; meta?: Record<string, unknown> },
  ): Promise<string> {
    if (!recordId.current) recordId.current = crypto.randomUUID();
    ...
    if (res.status === 401)
      throw new Error("请先登录后再启动驾驶舱（右上角 → 登录）");
    if (res.status === 402)
      throw new Error("积分不足，可在账户中心绑定自己的 API Key");
    if (!res.ok) throw new Error(d.error || "出错 " + res.status);
    setSavedRecord(d.sessionId);
    return d.text || "";
  }
  ...
  async function run() {
    ...
    setMode("run");
    const sset = (k: string, s: StageStatus) =>
      setStage((p) => ({ ...p, [k]: s }));
    const pushFeed = (who: string, color: string, text: string) =>
      setFeed((f) => [
        ...f,
        { who, color, text: (text || "").replace(/\s+/g, " ").slice(0, 160) },
      ]);
    const fly = (a: string, b: string, label: string, color: string) =>
    ...
    const collected: Record<
      string,
      { name: string; task: string; out: string }
    > = {};
    try {
      if (workersAll.length) {
        ...
        const plan = await ask(
          lead.system(MAJOR) +
            ...
            "\n" +
            ctx +
            `\n你是带头的总负责。本次工作流顺序：${workersAll.map((w) => w.name).join(" → ")}（其中「材料撰写」负责最后把全队成果打包成稿）。请用 2-3 句按此顺序点名分工调度，简洁有力。`,
          g,
          web,
        );
        ...
        if (dn) {
          setCaption(`${lead.name} 启动协作，把活交给 ${workers[0].name}`);
          await fly(leader, workers[0].key, "启动·交给你", lead.color);
        }
        let prior = "";
        for (let i = 0; i < dn; i++) {
          const m = workers[i];
          ...
          const out = await ask(
            m.system(MAJOR) +
              skillInstr(active, m.key) +
              "\n" +
              ctx +
              peer +
              `\n你是协同团队一员（工作流：${workersAll.map((w) => w.name).join("→")}）。只产出你负责的【${task}】部分，承接上游、为下游铺路；这部分约 ${LEN.sec} 字，要点充分展开、有数据/案例/论证，专业可直接写进 BP，不要过短。`,
            g + " —— 请完成：" + task,
            false,
            Math.min(3600, LEN.mt),
          );
          ...
          await wait(700);
          if (i < dn - 1) {
            setCaption(`${m.name} 把接力棒交给 ${workers[i + 1].name}`);
            await fly(m.key, workers[i + 1].key, "接力·交接", m.color);
          }
          sset(m.key, "done");
        }
        ...
        if (dn) {
          ...
          sset(leader, "think");
          const idxList = workers
            .map((m, i) => `${i + 1}. ${m.name}（${TASKMAP[m.key]}）`)
            .join("\n");
          const outList = workers
            .map((m, i) => `### ${i + 1}. ${m.name}\n${collected[m.key].out}`)
            .join("\n\n");
          let reviewRaw = "";
          try {
        ...
        if (writer) {
          ...
          const domainOut = workers
            .map(
              (m) =>
                `【${collected[m.key].name} · ${collected[m.key].task}】\n${collected[m.key].out}`,
            )
            .join("\n\n");
          const packed = await ask(
            writer.system(MAJOR) +
              skillInstr(active, "advisor") +
              "\n" +
              ctx +
          ...
          pushFeed(
            writer.name,
            writer.color,
            "汇总打包成完整 BP（约 " + packed.length + " 字）",
          );
          sset(writer.key, "write");
          await wait(700);
      ...
      setErr(e instanceof Error ? e.message : "运行中断");
      setCaption("");
    }
    setRunning(false);
  }
  ...
  async function doExport(kind: "word" | "pdf" | "pptx") {
    ...
    try {
      ...
      if (kind === "word") {
        ...
        logClientEvidence("export_doc", {
          title: `驾驶舱导出 · ${fn}`,
          payload: { fmt: kind, chars: draft.length },
        });
      } else if (kind === "pdf") {
        ...
        if (res.ok) {
          ...
          logClientEvidence("export_doc", {
            title: `驾驶舱导出 · ${fn}`,
            payload: { fmt: "pptx-lite", chars: draft.length },
          });
        } else {
          const d = await res.json().catch(() => ({}));
    ...
    try {
      ...
      const out = await ask(
        boss.system(MAJOR) +
          "\n" +
          ctx +
          `\n这是当前商业计划书全文：\n${draft}\n请严格按用户要求修改，并输出修改后的【完整】商业计划书（保持 Markdown「## 」分节，全文篇幅${LEN.total}、各节充实，不要只给片段、不要附加解释）。用户的修改要求：`,
        ins,
        web,
        LEN.mt,
        {
  ...
  async function saveCloud() {
    ...
    try {
      ...
      try {
        const r = localStorage.getItem(LS);
        if (r) sections = JSON.parse(r).sections || {};
      } catch {
        /* ignore */
      }
  ...
  function onLoadProject(p: CloudProject) {
    setName(p.name || "");
    setDraft(p.draft || "");
    const tm = (Array.isArray(p.team) ? p.team : []).filter(
      (k) => CREW_BY_KEY[k],
    );
    if (tm.length) {
      setTeam(tm);
      setLeader(tm.includes("boss") ? "boss" : tm[0]);
    }
    try {
      const r = localStorage.getItem(LS);
  ...
  const estChip = (
    <span
      style={{
        ...
        padding: "4px 10px",
        borderRadius: 999,
        background: "rgba(255,182,87,.16)",
        border: "1px solid rgba(255,182,87,.45)",
        color: dark ? "#ffd9a0" : "#9a6500",
        whiteSpace: "nowrap",
      }}
    >
      💰 预计消耗 ≈ <b>{estimate}</b> 积分起
      <span style={{ opacity: 0.8 }}>（含统筹/评审/汇总，打回重写每次+1）</span>
    </span>
  );
  const flowOrder = team.filter((k) => k !== leader).sort(byPhase);
  const flowChip = (k: string, label: string) => (
    <span
      style={{
        fontSize: 11,
        padding: "3px 8px",
        borderRadius: 999,
        background: `${CREW_BY_KEY[k].color}22`,
        border: `1px solid ${CREW_BY_KEY[k].color}55`,
        color: T.ink,
        whiteSpace: "nowrap",
      }}
    >
      {CREW_BY_KEY[k].emoji} {label}
    </span>
  );
  ...
  const workflowBar = (
    ...
    >
      <span style={{ fontSize: 11, color: T.mut, marginRight: 2 }}>
        🔀 工作流
      </span>
      ...
      {flowOrder.map((k) => (
        ...
        >
          {arrow}
          {flowChip(k, CREW_BY_KEY[k].name + (k === "advisor" ? "·打包" : ""))}
        </span>
      ))}
      {arrow}
      {flowChip(leader, "终审成稿")}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      {dark ? (
        <>
          <div
            ...
            style={{
              ...
              inset: 0,
              background:
                "radial-gradient(1200px 760px at 50% -12%, #0c1740, #070b22 52%, #04030f 100%)",
            }}
          />
          <div
            aria-hidden
        ...
        <div
          ...
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(1200px 600px at 70% -10%, #ffffff, #e3eafc 45%, #cfdaf4 100%)",
          }}
        />
      )}
      ...
      >
        ...
        >
          ...
          <b
            style={{
              ...
              letterSpacing: "-.3px",
              color: T.ink,
              textShadow: dark ? "0 2px 14px #000" : "none",
            }}
          >
            创业星舰 · 驾驶舱
          </b>
          ...
          >
            <Link
              ...
              style={{
                ...
                padding: "8px 15px",
                borderRadius: 999,
                background: "linear-gradient(120deg,#7aa8ff,#b388ff)",
                boxShadow: "0 4px 16px rgba(122,168,255,.45)",
              }}
            >
              🛒 技能商店
            </Link>
            <button
              ...
              title="切换 深色/浅色"
              style={{ ...ebtn, padding: "7px 12px", borderRadius: 999 }}
            >
              {dark ? "☀️ 浅色" : "🌙 深色"}
            </button>
        ...
        <div style={{ position: "relative", marginBottom: 12, color: T.ink }}>
          <Link href="/me/records">历史记录（退出后保留）</Link>{" "}
          {savedRecord && (
            <RecordExport
              endpoint={"/api/records/export?sessionId=" + savedRecord}
              label="导出驾驶舱完整记录"
            />
          )}
        </div>
        ...
        <div
          ...
          style={{
            position: "relative",
            height: narrow ? "min(50vh, 400px)" : "min(56vh, 520px)",
            minHeight: narrow ? 300 : 380,
            borderRadius: 18,
            border: `1px solid ${T.line}`,
            overflow: "hidden",
            marginBottom: 12,
        ...
        >
          <CrewStage
            agents={agents}
            flights={flights}
            onRemove={mode === "setup" ? toggleMember : undefined}
            dark={dark}
            caption={caption}
            feed={feed}
          />
                  ...
                  <div key={c.key}>
                    {on && (
                      ...
                      >
                        ...
                        >
                          {c.name}{" "}
                          <span style={{ color: "#9fb6e0", fontWeight: 400 }}>
                            · {c.nick}
                          </span>
                        </div>
                        <div
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: 0,
                        cursor: "pointer",
                        border: `2px solid ${on ? c.color : "rgba(120,200,255,.4)"}`,
                        background: "rgba(7,9,26,.5)",
                        boxShadow: on
                          ? `0 0 20px ${c.color}`
                          : "0 4px 12px #0008",
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          fontSize: 14,
                          fontWeight: 900,
                          border: "2px solid rgba(7,9,26,.8)",
                        }}
                      >
                        ＋
                      </span>
            ...
            >
              {outAgents.map((c) => (
                ...
                >
                  ...
                  >
                    <Ball k={c.key} emoji={c.emoji} color={c.color} size={30} />
                  </span>
        ...
        {done && (
          ...
          >
            ...
            <button onClick={reset} style={ebtn}>
              ↺ 重新协作
            </button>
          </div>
        )}
        ...
        {mode === "setup" && (
          <div style={{ ...hud, padding: 14, marginTop: 12 }}>
            ...
            <div style={{ marginTop: 10 }}>
              <AttachBar atts={atts} setAtts={setAtts} accent="#7aa8ff" />
            </div>
            ...
            >
              ...
              <button
                onClick={() => setWeb((w) => !w)}
                title="联网检索（需在 .env 配置搜索源后生效；未配置则用知识库作答）"
                style={{
                  ...ebtn,
                  borderColor: web ? "#34d399" : T.line,
                  color: web ? "#10b981" : T.ink,
                  background: web ? "rgba(52,211,153,.14)" : ebtn.background,
                }}
              >
                🌐 联网检索 {web ? "开" : "关"}
              </button>
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 15,
                  color: "#04030f",
                  cursor: !goal.trim() ? "default" : "pointer",
                  background: "linear-gradient(120deg,#7aa8ff,#b388ff)",
            ...
            {web && (
              <div style={{ fontSize: 11, color: T.mut, marginTop: 8 }}>
                ※ 联网检索需在服务器 .env 配置 SEARCH_API_URL / SEARCH_API_KEY
                后真正生效；未配置时本次自动改用知识库作答。
              </div>
            )}
            {err && (
              <div style={{ color: "#ef5e6e", fontSize: 12.5, marginTop: 8 }}>
                {err}
              </div>
            )}
            {cloudMsg && (
              <div
                style={{
                  color: cloudMsg.startsWith("✓") ? "#7ef0c0" : "#ffb066",
                  fontSize: 12.5,
                  marginTop: 8,
                }}
              >
        ...
        {mode === "run" && err && (
          <div
            style={{ ...hud, padding: 12, color: "#ef5e6e", fontSize: 12.5 }}
          >
            {err} ·{" "}
            <button
              onClick={reset}
              style={{ ...ebtn, padding: "3px 9px", marginLeft: 6 }}
            >
              返回
            </button>
          </div>
      ...
      {mode === "confirm" && (
        ...
        >
          ...
          >
            ...
            >
              <b style={{ color: CREW_BY_KEY[leader].color }}>
                {CREW_BY_KEY[leader].name}
              </b>{" "}
              将带 <b>{team.length - 1}</b> 位搭子，按以下工作流协同：
            </div>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              ...
              <button onClick={() => setMode("setup")} style={ebtn}>
                再想想
              </button>
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <b style={{ fontSize: 15, color: T.ink }}>
                {name || "商业计划书"} · 成稿
              </b>
              ...
              <button
                onClick={() => setDrawer(false)}
                style={{ ...ebtn, marginLeft: "auto", padding: "5px 11px" }}
              >
                ✕ 关闭
              </button>
            </div>
            ...
            >
              ...
              <button
                onClick={doRevise}
                disabled={revising || !revise.trim()}
                style={{
                  ...ebtn,
                  fontWeight: 700,
                  color: "#9fc2ff",
                  borderColor: "#7aa8ff66",
                  background: "rgba(122,168,255,.14)",
                  opacity: revising || !revise.trim() ? 0.55 : 1,
                }}
              >
                {revising ? "🐼 修改中…" : "✨ 继续打磨"}
              </button>
            ...
            >
              ...
              {pptLinks && (pptLinks.downloadUrl || pptLinks.editUrl) && (
                <>
                  {pptLinks.downloadUrl && (
                    <a
                      href={pptLinks.downloadUrl}
                      target="_blank"
                      rel="noreferrer"
                      style={{
                        fontSize: 12.5,
                  ...
                  {pptLinks.editUrl && (
                    <a
                      ...
                      style={{
                        ...
                        color: "#7ad0ff",
                        textDecoration: "none",
                        border: "1px solid rgba(122,208,255,.5)",
                        borderRadius: 999,
                        padding: "6px 13px",
                      }}
                    >
              ...
              <button
                onClick={saveCloud}
                disabled={saving}
                style={{ ...ebtn, color: "#9fc2ff", borderColor: "#7aa8ff66" }}
              >
                {saving ? "保存中…" : "☁ 保存到云"}
              </button>
              {cloudMsg && (
              ...
              {err && (
                <span style={{ color: "#ef5e6e", fontSize: 12 }}>{err}</span>
              )}
            </div>
          </div>
        </div>
      ...
      {intro && (
        <CockpitIntro
          onDone={() => {
            setIntro(false);
            try {
              sessionStorage.setItem(INTRO_SS, "1");
            } catch {
              /* ignore */
            }
          }}
      ...
      <Tour
        active={mode === "setup" && !intro}
        seenKey="cockpit_tour_v3"
        steps={COCKPIT_STEPS}
      />
